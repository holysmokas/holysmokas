import"./modulepreload-polyfill-B5Qt9EMX-ulV_1b7r-BGxEsOWE.js";import"./firebase-BtvxRjGj-Bmveahn2-DtNkYIDR.js";/* empty css                                */const k="https://script.google.com/macros/s/AKfycbzJOYtbHJRuIgQTT7BVhmzGvRaJ1TMBd-VDoe_DUk_Rbnyr9EBiB5W9Xutir8r8LcgUTg/exec";function p(t,e,i=!0){const o=document.getElementById("formModal"),n=document.getElementById("modalTitle"),r=document.getElementById("modalMessage"),s=document.getElementById("modalIcon");n.textContent=t,r.textContent=e,s.textContent=i?"✔":"✕",s.style.background=i?"var(--secondary)":"#ef4444",o.classList.add("show")}window.closeModal=function(){document.getElementById("formModal").classList.remove("show")};async function E(t){try{const e=await fetch(k,{method:"POST",mode:"no-cors",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});return console.log("Data sent to Google Apps Script"),!0}catch(e){return console.error("Error sending to Google Script:",e),!1}}function S(t){let e=t.replace(/^https?:\/\//,"");return e=e.replace(/^www\./,""),e=e.split("/")[0],e=e.trim(),e}window.checkDomain=async function(){const t=document.getElementById("domainInput"),e=document.getElementById("domainResult"),i=t.value.trim();if(!i){e.innerHTML="⚠️ Please enter a domain name.",e.style.color="#f59e0b";return}e.innerHTML="🔍 Checking availability...",e.style.color="#6b7280";try{const o=await(await fetch("http://localhost:8080/check-domain",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({domain:i})})).json();if(o.available===!0){const n=o.cleanedDomain||S(i),r=o.pricing?.initialCost||o.price||12.99,s=o.pricing?.renewalCost||o.renewalPrice||12.99;e.innerHTML=`
            ...
            `,e.style.color="#10b981",window.selectedDomain=n,window.domainPricing={initialCost:r,renewalCost:s}}else{const n=o.domain||i;e.innerHTML=`
        ...
        `,e.style.color="#ef4444"}};window.addDomainToForm=function(t,e=0,i=0){const o=document.getElementById("currentUrl");o&&(o.value=`https://${t}`,o.style.border="2px solid #10b981",setTimeout(()=>{o.style.border=""},2e3)),window.selectedDomain=t,window.domainPricing={initialCost:e,renewalCost:i};const n=document.getElementById("domainResult");n.innerHTML=`
        <div style="background: #f0fdf4; border: 2px solid #10b981; border-radius: 8px; padding: 1rem; margin-top: 1rem;">
            <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.75rem;">
                <span style="font-size: 1.5rem;">✅</span>
            ...
            <div style="background: white; padding: 0.75rem; border-radius: 6px; margin-bottom: 0.75rem;">
                ...
                <div style="display: flex; justify-content: space-between;">
                    <span style="color: #6b7280;">Annual Renewal:</span>
                    <strong style="color: #059669;">$${i.toFixed(2)}/year</strong>
                </div>
            </div>
    ...
    `,n.style.color="#10b981",m(),document.getElementById("contactForm").scrollIntoView({behavior:"smooth",block:"start"}),setTimeout(()=>{const r=document.getElementById("businessName");r&&!r.value&&r.focus()},500)};function g(t){return{"Starter - $599 (New Website)":599,"Business - $1,199 (New Website)":1199,"Small Shop - $1,699 (New Website)":1699,"E-Commerce/Enterprise - Custom Quote":0,"Website Redesign/Updates":0,"Not Sure Yet":0}[t]||0}function m(){const t=document.getElementById("package"),e=t?t.value:"",i=g(e),o=window.domainPricing?window.domainPricing.initialCost:0;let n=document.getElementById("pricingBreakdown");if(!n){n=document.createElement("div"),n.id="pricingBreakdown",n.style.cssText="margin-top: 2rem; padding: 1.5rem; background: #f0fdf4; border: 2px solid #10b981; border-radius: 8px;";const r=document.getElementById("submitBtn");r&&r.parentNode.insertBefore(n,r)}if(i>0){const r=i+o;n.innerHTML=`
