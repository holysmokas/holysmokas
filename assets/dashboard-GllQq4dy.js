import"./modulepreload-polyfill-B5Qt9EMX.js";import{o as I,a as v,E as m}from"./config-D-CCVyFk.js";/* empty css              */let l=null,f=null;I(v,async e=>{e?(l=e,await E(e.uid)):window.location.href="./login.html"});window.auth=v;function $(e,t=3){return!e||e.length===0?'<p class="no-modifications">No modifications yet</p>':e.slice(0,t).map((o,n)=>{const a=n+1,d=o.status==="completed"?"completed":o.status==="pending"?"pending":"failed",r=o.userEmail||o.userName||"User",p=o.timestamp?new Date(o.timestamp).toLocaleString():"Unknown date";return`
            <div class="log-entry">
        ...
        `}).join("")}async function E(e){try{const s=await(await fetch(m.userProjects(e))).json(),o=document.getElementById("projectsContainer");s.success&&s.projects.length>0?o.innerHTML=s.projects.map(n=>{const a=n.modificationsUsed||0,d=n.modificationsLimit||3,r=d-a,p=r>0,i=n.modifications||[];return`
...
`;document.head.appendChild(k);document.addEventListener("DOMContentLoaded",()=>{console.log("🚀 DriveUI: Initialized"),D()});window.checkDriveStatus=w;
