...
export async function enablePages(repoName) {
  try {
    ...
    console.log("🌐 GitHub Pages enabled with GitHub Actions");
  } catch (err) {
    if (err.message.includes("already exists")) {
      console.log("✅ GitHub Pages already enabled");
    } else {
      console.error("⚠️ Error enabling GitHub Pages:", err.message);
    }
  }
}
...
export async function deployToGitHub(projectDir, repoName, description = "Generated by HolySmokas") {
  ...
  try {
    ...
    console.log("⏳ Waiting 3 seconds for GitHub initialization...");
    await new Promise(resolve => setTimeout(resolve, 3000));

    if (fs.existsSync(workflowPath)) {
      console.log("📋 Temporarily removing workflow file from git push...");
      workflowContent = fs.readFileSync(workflowPath, "utf8");
      fs.unlinkSync(workflowPath);
    }

    console.log("📤 Pushing project files (without workflow)...");
    ...
    console.log("✅ Project files pushed successfully");

    console.log("⏳ Waiting 8 seconds for GitHub to process all files...");
    ...
    if (workflowContent) {
      ...
      console.log("✅ Workflow file added - GitHub Actions will now deploy");
    }

    console.log("⏳ Waiting 3 seconds before enabling Pages...");
    await new Promise(resolve => setTimeout(resolve, 3000));
...
export async function generateProjectCode(config) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    console.warn("⚠️ Missing OPENAI_API_KEY — returning default fallback code.");
    return `
      // App.jsx
      import React from 'react';
