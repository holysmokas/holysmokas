import"./modulepreload-polyfill-B5Qt9EMX.js";import{o as M,a as w,E as m}from"./config-LMHd4xPy.js";/* empty css              */let l=null,f=null;M(w,async e=>{e?(l=e,await E(e.uid)):window.location.href="./login.html"});window.auth=w;function I(e,t=3){return!e||e.length===0?'<p class="no-modifications">No modifications yet</p>':e.slice(0,t).map((o,n)=>{const r=n+1,d=o.status==="completed"?"completed":o.status==="pending"?"pending":"failed",a=o.userEmail||o.userName||"User",u=o.timestamp?new Date(o.timestamp).toLocaleString():"Unknown date";return`
            <div class="log-entry">
        ...
        `}).join("")}async function E(e){try{const s=await(await fetch(m.userProjects(e))).json(),o=document.getElementById("projectsContainer");s.success&&s.projects.length>0?o.innerHTML=s.projects.map(n=>{const r=n.modificationsUsed||0,d=n.modificationsLimit||3,a=d-r,u=a>0,i=n.modifications||[];return`
    ...
    `,document.body.appendChild(o),setTimeout(()=>{o.style.animation="slideOut 0.3s ease-in",setTimeout(()=>o.remove(),300)},4e3)}const $=document.createElement("style");$.textContent=`
    @keyframes slideIn {
...
`;document.head.appendChild($);document.addEventListener("DOMContentLoaded",()=>{console.log("🚀 DriveUI: Initialized"),B()});window.checkDriveStatus=v;
