import"./modulepreload-polyfill-B5Qt9EMX.js";import{o as $,a as I,E as u}from"./config-LMHd4xPy.js";/* empty css              */let m=null,y=null;$(I,async e=>{e?(m=e,await S(e.uid)):window.location.href="./login.html"});window.auth=I;const b=new URLSearchParams(window.location.search);if(b.get("connect_success")==="true"){const e=b.get("project_id");console.log("✅ Connect onboarding completed for project:",e),setTimeout(()=>{alert("🎉 Payment setup complete! Your shop can now accept payments."),window.history.replaceState({},document.title,window.location.pathname)},500)}if(b.get("connect_refresh")==="true"){const e=b.get("project_id");console.log("🔄 Connect onboarding needs refresh for project:",e),setTimeout(()=>{e&&typeof setupPayments=="function"&&setupPayments(e)},1e3)}function C(e,t=3){return!e||e.length===0?'<p class="no-modifications">No modifications yet</p>':e.slice(0,t).map((o,n)=>{const s=n+1,d=o.status==="completed"?"completed":o.status==="pending"?"pending":"failed",l=o.userEmail||o.userName||"User",p=o.timestamp?new Date(o.timestamp).toLocaleString():"Unknown date";return`
        ...
        `}).join("")}async function S(e){try{const i=await(await fetch(u.userProjects(e))).json(),o=document.getElementById("projectsContainer");if(i.success&&i.projects.length>0){const n=await Promise.all(i.projects.map(async s=>{const d=s.modificationsUsed||0,l=s.modificationsLimit||3,p=l-d,r=p>0,a=s.modifications||[],E=s.packageType&&s.packageType.toLowerCase().includes("smallshop");let g=null;E&&(g=await checkConnectStatus(s.id));let h="";return E&&(g&&g.connected&&g.status==="active"?h=`
    ...
    `};window.closeAiModificationModal=function(){document.getElementById("aiModificationModal").classList.remove("show"),y=null};window.submitAiModification=async function(e){e.preventDefault();const t=document.getElementById("aiModificationInput"),i=t.value.trim(),o=document.getElementById("aiSubmitBtn"),n=document.getElementById("aiChatMessages"),s=document.getElementById("aiProcessingMessage"),d=document.getElementById("aiModificationForm");if(console.log("🔍 DEBUG: submitAiModification called"),console.log("🔍 currentUser:",m),console.log("🔍 currentProject:",y),console.log("🔍 request:",i),console.log("🔍 ENDPOINTS:",u),!i||!y)return;const l=document.createElement("div");l.className="user-message",l.innerHTML=`<strong>You:</strong> ${i}`,n.appendChild(l),n.scrollTop=n.scrollHeight,t.value="",o.disabled=!0,d.style.display="none",s.style.display="block";try{const r=await(await fetch(u.requestModification,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:m.uid,projectId:y.id,modificationRequest:i})})).json();if(s.style.display="none",d.style.display="block",o.disabled=!1,r.success){const a=document.createElement("div");a.className="ai-message",a.innerHTML=`<strong>AI Assistant:</strong> Great! I've processed your request and updated your website. The changes are being deployed now and should be live in 2-3 minutes. ${r.modificationsRemaining} free modification${r.modificationsRemaining!==1?"s":""} remaining.`,n.appendChild(a),n.scrollTop=n.scrollHeight,document.getElementById("modificationsCount").textContent=r.modificationsRemaining,setTimeout(()=>{S(m.uid),r.modificationsRemaining===0&&(closeAiModificationModal(),B("Modifications Limit Reached","You've used all 3 free modifications. Additional changes will require a small fee. Contact us at (415) 691-7085 to discuss pricing."))},2e3)}else{const a=document.createElement("div");a.className="ai-message",a.innerHTML=`<strong>AI Assistant:</strong> I'm sorry, there was an error processing your request: ${r.error||"Unknown error"}. Please try again or contact support at (415) 691-7085.`,n.appendChild(a),n.scrollTop=n.scrollHeight}}catch(p){console.error("Error submitting modification:",p),s.style.display="none",d.style.display="block",o.disabled=!1;const r=document.createElement("div");r.className="ai-message",r.innerHTML="<strong>AI Assistant:</strong> I encountered a technical error. Please try again or contact support at (415) 691-7085.",n.appendChild(r),n.scrollTop=n.scrollHeight}};function B(e,t){const i=document.getElementById("responseModal"),o=document.getElementById("responseContent");o.innerHTML=`
        <h2>${e}</h2>
        <p>${t}</p>
    ...
    `,document.body.appendChild(o),setTimeout(()=>{o.style.animation="slideOut 0.3s ease-in",setTimeout(()=>o.remove(),300)},4e3)}const P=document.createElement("style");P.textContent=`
    @keyframes slideIn {
...
`;document.head.appendChild(P);document.addEventListener("DOMContentLoaded",()=>{console.log("🚀 DriveUI: Initialized"),x()});window.checkDriveStatus=k;
