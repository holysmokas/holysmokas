import"./modulepreload-polyfill-B5Qt9EMX.js";import{o as M,a as v,E as m}from"./config-Ci-ZL9Yi.js";/* empty css              */let l=null,f=null;M(v,async e=>{e?(l=e,await $(e.uid)):window.location.href="./login.html"});window.auth=v;function I(e,t=3){return!e||e.length===0?'<p class="no-modifications">No modifications yet</p>':e.slice(0,t).map((o,n)=>{const a=n+1,d=o.status==="completed"?"completed":o.status==="pending"?"pending":"failed",r=o.userEmail||o.userName||"User",p=o.timestamp?new Date(o.timestamp).toLocaleString():"Unknown date";return`
            <div class="log-entry">
                ...
                <p class="log-user">👤 Requested by: ${r}</p>
            </div>
        `}).join("")}async function $(e){try{const s=await(await fetch(m.userProjects(e))).json(),o=document.getElementById("projectsContainer");s.success&&s.projects.length>0?o.innerHTML=s.projects.map(n=>{const a=n.modificationsUsed||0,d=n.modificationsLimit||3,r=d-a,p=r>0,i=n.modifications||[];return`
                <div class="project-card">
                    ...
                    <div class="project-info">
                        ...
                        <p><strong>Modifications:</strong> 
                            <span class="${r>0?"text-success":"text-warning"}">
                                ${a}/${d} used
                            </span>
                            ${r===0?'<span class="badge-limit">Limit Reached</span>':""}
                        </p>
                    </div>
                    ...
                    <div class="project-btn-group">
                        ...
                        ${p?`
                            <button 
                                ...
                                🤖 Request Modification (${r} free left)
                            </button>
                        `:`
                            <button 
    ...
    `};window.closeAiModificationModal=function(){document.getElementById("aiModificationModal").classList.remove("show"),f=null};window.submitAiModification=async function(e){e.preventDefault();const t=document.getElementById("aiModificationInput"),s=t.value.trim(),o=document.getElementById("aiSubmitBtn"),n=document.getElementById("aiChatMessages"),a=document.getElementById("aiProcessingMessage"),d=document.getElementById("aiModificationForm");if(console.log("🔍 DEBUG: submitAiModification called"),console.log("🔍 currentUser:",l),console.log("🔍 currentProject:",f),console.log("🔍 request:",s),console.log("🔍 ENDPOINTS:",m),!s||!f)return;const r=document.createElement("div");r.className="user-message",r.innerHTML=`<strong>You:</strong> ${s}`,n.appendChild(r),n.scrollTop=n.scrollHeight,t.value="",o.disabled=!0,d.style.display="none",a.style.display="block";try{const i=await(await fetch(m.requestModification,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:l.uid,projectId:f.id,modificationRequest:s})})).json();if(a.style.display="none",d.style.display="block",o.disabled=!1,i.success){const u=document.createElement("div");u.className="ai-message",u.innerHTML=`<strong>AI Assistant:</strong> Great! I've processed your request and updated your website. The changes are being deployed now and should be live in 2-3 minutes. ${i.modificationsRemaining} free modification${i.modificationsRemaining!==1?"s":""} remaining.`,n.appendChild(u),n.scrollTop=n.scrollHeight,document.getElementById("modificationsCount").textContent=i.modificationsRemaining,setTimeout(()=>{$(l.uid),i.modificationsRemaining===0&&(closeAiModificationModal(),k("Modifications Limit Reached","You've used all 3 free modifications. Additional changes will require a small fee. Contact us at (415) 691-7085 to discuss pricing."))},2e3)}else{const u=document.createElement("div");u.className="ai-message",u.innerHTML=`<strong>AI Assistant:</strong> I'm sorry, there was an error processing your request: ${i.error||"Unknown error"}. Please try again or contact support at (415) 691-7085.`,n.appendChild(u),n.scrollTop=n.scrollHeight}}catch(p){console.error("Error submitting modification:",p),a.style.display="none",d.style.display="block",o.disabled=!1;const i=document.createElement("div");i.className="ai-message",i.innerHTML="<strong>AI Assistant:</strong> I encountered a technical error. Please try again or contact support at (415) 691-7085.",n.appendChild(i),n.scrollTop=n.scrollHeight}};function k(e,t){const s=document.getElementById("responseModal"),o=document.getElementById("responseContent");o.innerHTML=`
        <h2>${e}</h2>
        <p>${t}</p>
    ...
    `,t.classList.add("show")};window.purchaseModifications=async function(e,t){console.log(`Purchase request: ${t} modifications for project ${e}`),document.getElementById("responseModal");const s=document.getElementById("responseContent");s.innerHTML=`
        <h2>💳 Complete Your Purchase</h2>
        <p>To purchase ${t==="unlimited"?"unlimited":t} modification${t!==1&&t!=="unlimited"?"s":""}, please contact us:</p>
    ...
    `};window.openSupportModal=function(){document.getElementById("supportModal").classList.add("show"),document.getElementById("supportSubject").value="",document.getElementById("supportMessage").value="",document.getElementById("supportForm").style.display="block",document.getElementById("supportProcessing").style.display="none"};window.closeSupportModal=function(){document.getElementById("supportModal").classList.remove("show")};window.submitSupportRequest=async function(e){e.preventDefault();const t=document.getElementById("supportSubject").value.trim(),s=document.getElementById("supportMessage").value.trim(),o=document.getElementById("supportSubmitBtn"),n=document.getElementById("supportForm"),a=document.getElementById("supportProcessing");if(!(!t||!s)){n.style.display="none",a.style.display="block",o.disabled=!0;try{const r=await(await fetch(m.supportRequest||`${m.requestModification.replace("/request-modification","/support-request")}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:l.uid,userEmail:l.email,subject:t,message:s,userName:l.displayName||l.email,timestamp:new Date().toISOString()})})).json();if(a.style.display="none",r.success){const i=document.getElementById("supportModal").querySelector(".modal-box");i.innerHTML=`
            ...
            `}else n.style.display="block",o.disabled=!1,alert("Error sending message: "+(r.error||"Unknown error. Please try calling us at (415) 691-7085"))}catch(d){console.error("Error submitting support request:",d),a.style.display="none",n.style.display="block",o.disabled=!1,alert("Technical error sending message. Please call us directly at (415) 691-7085")}}};const y=m?.apiUrl||(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://localhost:8080":"https://api.holysmokas.com");let c=null,h=null;M(v,async e=>{e?(c=e.uid,window.currentUserId=e.uid,console.log("🔐 DriveUI: User authenticated:",e.uid),await w()):(c=null,g({connected:!1}))});async function w(){if(!c){console.log("⚠️ DriveUI: No user ID, skipping status check"),g({connected:!1});return}try{console.log("🔍 Checking Drive status for user:",c);const t=await(await fetch(`${y}/api/drive/status?userId=${c}`)).json();console.log("📡 Drive status response:",t),t.success?g(t):g({connected:!1})}catch(e){console.error("Error checking Drive status:",e),g({connected:!1,error:e.message})}}function g(e){const t=document.getElementById("driveSection");if(!t){console.warn("⚠️ DriveUI: #driveSection element not found");return}e.connected?t.innerHTML=`
        ...
        `}window.connectDrive=function(){if(!c){alert("Please log in first");return}const e=h||"none";console.log("🔗 Starting Drive OAuth flow..."),console.log("   User ID:",c),console.log("   Project ID:",e),window.location.href=`${y}/auth/google/drive/connect?userId=${c}&projectId=${e}`};window.disconnectDrive=async function(){if(confirm("Are you sure you want to disconnect Google Drive? Your uploaded images will still work.")){if(!c){alert("Please log in first");return}try{const t=await(await fetch(`${y}/api/drive/disconnect`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:c})})).json();t.success?(b("Google Drive disconnected successfully","success"),await w()):alert("Error: "+t.error)}catch(e){console.error("Error disconnecting Drive:",e),alert("Failed to disconnect Google Drive")}}};window.uploadProductImage=async function(e,t,s){if(!c)throw new Error("Please log in first");if(!h)throw new Error("No project selected");const o=new FormData;o.append("image",e),o.append("userId",c),o.append("projectId",h),o.append("productId",t),o.append("productName",s);const a=await(await fetch(`${y}/api/upload-product-image`,{method:"POST",body:o})).json();if(!a.success)throw a.needsConnection?(confirm("You need to connect Google Drive to upload images. Connect now?")&&window.connectDrive(),new Error("Drive not connected")):new Error(a.error);return a};window.setCurrentProject=function(e){h=e,window.currentProjectId=e};function B(){const e=new URLSearchParams(window.location.search);if(e.get("drive_connected")==="true"&&(b("✅ Google Drive connected successfully!","success"),window.history.replaceState({},"",window.location.pathname)),e.get("drive_error")){const t=e.get("drive_error");b("❌ Drive connection failed: "+decodeURIComponent(t),"error"),window.history.replaceState({},"",window.location.pathname)}}function b(e,t="info"){const s=document.querySelector(".drive-notification");s&&s.remove();const o=document.createElement("div");o.className=`drive-notification notification-${t}`,o.textContent=e,o.style.cssText=`
        position: fixed;
        top: 20px;
        right: 20px;
        padding: 1rem 1.5rem;
    ...
    `,document.body.appendChild(o),setTimeout(()=>{o.style.animation="slideOut 0.3s ease-in",setTimeout(()=>o.remove(),300)},4e3)}const E=document.createElement("style");E.textContent=`
    @keyframes slideIn {
...
`;document.head.appendChild(E);document.addEventListener("DOMContentLoaded",()=>{console.log("🚀 DriveUI: Initialized"),B()});window.checkDriveStatus=w;
